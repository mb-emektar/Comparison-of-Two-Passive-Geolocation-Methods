function output = radar_range(xradar,yradar,zradar,...
    xradar_dot,yradar_dot,zradar_dot,...
    v_xplane,v_yplane,v_zplane,...
    v_xplane_dot,v_yplane_dot,v_zplane_dot)
%% Designing a Basic Monostatic Pulse Radar
% This example shows how to design a monostatic pulse radar to estimate the
% target range. A monostatic radar has the transmitter collocated with
...
max_range = 100000;    % Maximum unambiguous range
...
sensormotion = phased.Platform(...
    'InitialPosition',[xradar;yradar;zradar],...
    'Velocity',[xradar_dot;yradar_dot;zradar_dot]);
...
tgtpos = [v_xplane;v_yplane;v_zplane];
tgtvel = [v_xplane_dot;v_yplane_dot;v_zplane_dot];
...
% <docid:phased_examples.example-ex12077916> for an example using a chirp
% waveform.
end